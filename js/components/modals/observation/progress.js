...
const ObsProgress = {
                    ...
                    <div class="border rounded-lg overflow-hidden transition-all ${bgClass}" id="card-${m.id}">
                        ...
                        <div id="prog-opts-${m.id}" class="hidden bg-slate-50 p-2 border-t border-slate-100 grid grid-cols-4 gap-2">
                            ${[1,2,3,4].map(s => {
                                ...
                                return `<button onclick="ObsProgress.selectScore('${m.id}', ${s})" class="p-2 rounded border text-[10px] font-bold transition ${active}">${Utils.getScoreLabel(s)}</button>`;
                            }).join('')}
                        </div>
                    </div>`;
                });
    ...
    selectScore: (id, score) => {
        if (ObsProgress.scores[id] === score) {
            delete ObsProgress.scores[id]; // Toggle Off
        } else {
            ObsProgress.scores[id] = score; // Select
        }
        
        // Re-render this item only is tricky without React.
        // Fast path: Update DOM directly based on state
        const current = ObsProgress.scores[id];
        const badge = document.getElementById(`badge-${id}`);
        const card = document.getElementById(`card-${id}`);
        
        if(current) {
            badge.innerText = Utils.getScoreLabel(current);
            badge.classList.remove('hidden');
            card.className = "border rounded-lg overflow-hidden transition-all bg-emerald-50 border-emerald-200";
            document.getElementById(`prog-opts-${id}`).classList.add('hidden'); // Close on select
        } else {
