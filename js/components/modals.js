const Modals = {
    ...
    renderAccordion: () => {
        ...
        Object.keys(g).forEach((d, i) => {
            let h = `<div class="border rounded-xl bg-white overflow-hidden mb-2"><button onclick="Modals.toggleAcc('d${i}')" class="w-full text-left p-4 font-bold text-sm text-slate-700 flex justify-between bg-slate-50">${CONFIG.DOMAINS[d]}<i id="icon-d${i}" class="fa-solid fa-chevron-down transition-transform"></i></button><div id="d${i}" class="accordion-content px-4">`;
