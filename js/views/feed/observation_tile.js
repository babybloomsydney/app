const FeedCard_Observation = {
    render: (item) => {
        ...
        return `<div class="feed-item bg-white p-5 rounded-2xl shadow-sm border border-slate-100 mb-4"><div class="flex justify-between items-center mb-3"><div class="flex items-center gap-2"><div class="w-6 h-6 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center text-xs"><i class="fa-solid fa-eye"></i></div><span class="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Observation</span></div><span class="text-[10px] text-gray-400">${date}</span></div><div class="mb-3"><div class="text-[10px] font-bold uppercase mb-2 flex flex-wrap gap-1">${displayDomains}</div>${mDesc ? `<p class="text-sm font-bold text-slate-800 mb-2">${mDesc}</p>` : ''}<p class="text-sm text-slate-600 leading-relaxed">${item.data.note}</p></div>${score ? `<div class="inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-50 text-emerald-700 rounded-lg border border-emerald-100"><i class="fa-solid fa-chart-simple text-xs"></i><span class="text-xs font-bold uppercase">Achieved: ${score}</span></div>` : ''}</div>`;
    }
};
